...
var signal_emitted = false  # Flag to track if the signal has been emitted

@onready var canvas_layer = $CanvasLayer
@onready var progress_bar = $CanvasLayer/VBoxContainer/ProgressBar
...
func reset_cube(body):
	...
	signal_emitted = false  # Reset the signal emitted flag
...
func _process(delta):
	if player_inside:
		...
		if Input.is_action_pressed("latter_button") and not signal_emitted:
			...
			progress_timer = 0.0
			progress_bar.value = 0
			signal_emitted = false  # Reset the signal emitted flag once the button is released
